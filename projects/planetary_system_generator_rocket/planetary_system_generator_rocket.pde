...
void draw() {
  ...
  if (rocketCam) {
    camera(rocket.getRocketPosition().x, rocket.getRocketPosition().y, rocket.getRocketPosition().z, 
      width, height/2, rocket.getRocketPosition().z, 0, 1, 0); 
  }else{
...
void help() {
  ...
  if (rocketCam) {
    text("Press 'e' to exit rocket", -(width/2)+20, -(height/2)+30);
  } else {
    text("Press 'c' to enter rocket", -(width/2)+20, -(height/2)+30);
  }
...
void keyPressed() {
  if (key == 'c') {
    println("Entrar al cohete");
    rocketCam = true;
  } else if (key == 'e') {
    println("Salir del cohete");
    rocketCam = false;
  }
}
